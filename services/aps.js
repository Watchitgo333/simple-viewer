...
console.log(APS_BUCKET, APS_CLIENT_ID, APS_CLIENT_SECRET)

let internalAuthClient = new APS.AuthClientTwoLegged(APS_CLIENT_ID, APS_CLIENT_SECRET, ['bucket:create', 'bucket:read', 'data:write', 'data:create', 'data:read'], true);
...
service.getInternalToken = async () => {
 if(!internalAuthClient.isAuthorized()) {
    ...
    // console.log(await internalAuthClient.authenticate())
 }
...
service.ensureBucketExists = async (bucketKey) => {
    try {
        let resp = await new APS.BucketsApi().getBucketDetails(bucketKey, null, await service.getInternalToken());
    } catch (err) {
